#!/usr/bin/env python
# -*- coding: utf-8 -*-
# File              : ampel/contrib/ztfbh/t2/T2SimpleMetrics.py
...
def _lc_to_dict(lc):
    ...
    keys = (
        'jd', 'fid',
        'pid', 'diffmaglim', 'pdiffimfilename', 'programpi',
        'programid', 'candid', 'isdiffpos', 'tblid', 'nid',
        'rcid', 'field', 'xpos', 'ypos', 'ra', 'dec',
        'magpsf', 'sigmapsf', 'chipsf', 'magap', 'sigmagap',
        'distnr', 'magnr', 'sigmagnr', 'chinr', 'sharpnr',
        'sky', 'magdiff', 'fwhm', 'classtar', 'mindtoedge',
        'magfromlim', 'seeratio', 'aimage', 'bimage',
        'aimagerat', 'bimagerat', 'elong', 'nneg', 'nbad',
        'rb', 'rbversion', 'drb', 'drbversion', 'ssdistnr',
        'ssmagnr', 'ssnamenr', 'sumrat', 'magapbig', 'sigmagapbig',
        'ranr', 'decnr', 'ndehist', 'ncovhist', 'jdstarhist',
        'jdendhist', 'scorr', 'tooflag', 'objectidps1',
        'sgmag1', 'srmag1', 'simag1', 'szmag1', 'sgscore1',
        'distpsnr1', 'objectidps2', 'sgmag2', 'srmag2',
        'simag2', 'szmag2', 'sgscore2', 'distpsnr2', 'objectidps3',
        'sgmag3', 'srmag3', 'simag3', 'szmag3', 'sgscore3', 'distpsnr3',
        'nmtchps', 'rfid', 'jdstartref', 'jdendref', 'nframesref',
        'dsnrms', 'ssnrms', 'dsdiff', 'magzpsci', 'magzpsciunc',
        'magzpscirms', 'nmatches', 'clrcoeff', 'clrcounc',
        'zpclrcov', 'zpmed', 'clrmed', 'clrrms', 'neargaia',
...
def compute(meta_data, lc, logger, verbose=False):
    ...
    data_dict1 = {key:value for key, value in lc.items() if len(value)==len(lc['jd'])}
    ...
    ipeak = photoarr['magpsf'] == min(photoarr['magpsf']) # not very robust but OK for now
    ...
    for flt in [1,2]:
        ...
        if result['ndetections_{0}'.format(flt_name)] >2:
            ...
            dra = (photoarr_flt['ra']-photoarr_flt['ranr'])*3600 # offset to nearest ZTF PSF catalog source
            ...
            sigma_offset = np.clip(0.24 + 0.04 * (photoarr_flt['magpsf']-20), 0.1, 1)   # equation from the NedStark paper
            snr_weight = 1/sigma_offset**2

            if 'magnr' in photoarr.columns:
                sigma_offset_nr = np.clip(0.24 + 0.04 * (np.median(photoarr_flt['magnr'])-20), 0.1, 1) #/ np.sqrt(5) # assume x images make a ref frame
            else:
                sigma_offset_nr = 0.05

            dunc_theo = np.sqrt(1/(sum(1/sigma_offset**2)) + sigma_offset_nr**2)    # can be too optimistic if large number of observations
            dunc_rms = np.sqrt(dstd**2 + sigma_offset_nr**2)                        # can be too optimisiric if small number of observations
            dunc = np.max( [dunc_rms,dunc_theo] )

            weighted_dra = wmean(dra,   snr_weight)
            weighted_dec = wmean(ddec, snr_weight)
            logger.info('weighted (dra,drec) {0:0.5f} {1:0.5f}'.format(weighted_dra, weighted_dec))
            ...
            result['offset_med_{0}'.format(flt_name)] = np.sqrt(np.median(dra)**2+np.median(ddec)**2)
            result['offset_weighted_{0}'.format(flt_name)] = offset_weighted
            result['offset_sig_{0}'.format(flt_name)] = offset_weighted /dunc

            logger.info(str(flt_name) + ': ' + 'simple_metrics: distance to ref (median, weighted, significance): {0:0.3f} {1:0.3f} {2:0.3f}'.format(result['offset_med_{0}'.format(flt_name)], result['offset_weighted_{0}'.format(flt_name)], result['offset_sig_{0}'.format(flt_name)]))
            logger.info(str(flt_name) + ': ' + 'simple_metrics: distance to ref (rms, unc_rm, unc_theo) : {0:0.3f} {1:0.3f} {2:0.3f}'.format(result['offset_rms_{0}'.format(flt_name)], dunc_rms, dunc_theo))
    ...
    result['lastest_obs_is_detection'] = np.int(max(photoarr0['jd'])==result['latest_jd'])
    result['latest_maglim'] = min(photoarr[ilatest]['diffmaglim']) #assumed limmag meant diffmaglim
    ...
    for flt in (1,2):
        ...
        if sum(ii):
            ...
            result['peak_diff_mag_'+flt_name]   = ((photoarr['magpsf'][ii]-result['ref_mag_'+flt_name])[ip]).values[0]
            result['latest_diff_mag_'+flt_name] = ((photoarr['magpsf'][ii]-result['ref_mag_'+flt_name])[il]).values[0]
            logger.info(str(flt_name) + ': ' + '(ref_mg, peak_diff_mag)  = ({0:0.2f}, {1:0.2f})'.format(result['ref_mag_'+flt_name], result['peak_diff_mag_'+flt_name]))
        else:
    ...
    result['latest_diff_mag'] = min(result['latest_diff_mag_r'], result['latest_diff_mag_g'])
    result['peak_diff_mag'] = min(result['peak_diff_mag_r'], result['peak_diff_mag_g'])

    if ('ranr' in photoarr.columns):
        plot_info['offset_plot'] = True
        plot_info['dra'] = (photoarr['ra']-photoarr['ranr'])*3600 # offset to nearest ZTF PSF catalog source
        plot_info['ddec'] = (photoarr['dec']-photoarr['decnr'])*3600

        sigma_offset = np.clip(0.24 + 0.04 * (photoarr['magpsf']-20), 0.1, 1)   # equation from the NedStark paper
