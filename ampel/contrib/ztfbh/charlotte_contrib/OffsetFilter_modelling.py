...
class OffsetFilter(AbsAlertFilter):
	...
	def __init__(self, on_match_t2_units, base_config=None, run_config=None, logger=None):
		...
		self.brightPS1Mag = { k : run_config['BrightPS1'+k+'Mag'] for k in ['g','r','i','z'] }
		...
		self._default_filters = [
			isdetect_flt,distnr_flt, nbad_flt, fwhm_flt, 
			elong_flt, magdiff_flt, isdiff_flt1, isdiff_flt2
		]
	...
	def alert_to_Gaia(self, alert, force_ztf=False):
		...
		if (min([ps1_mag[k] for k in ps1_mag])<0) or force_ztf:
			if not force_ztf:
				self.logger.warning("no decent PS1 photometry ({0:0.2f} {1:0.2f} {2:0.2f}), trying to use ZTF to estimate Gaia flux".format(ps1_mag['g'], ps1_mag['r'], ps1_mag['i']))

			magnr_g = alert.get_values('magnr', filters={'attribute':'fid','operator':'==','value':1})
			magnr_R = alert.get_values('magnr', filters={'attribute':'fid','operator':'==','value':2})
			...
			if len(magnr_g) and len(magnr_R):
				...
				self.logger.warning("not enough ZTF detections (g={0}, R={1}) to compute Gaia mag".format(len(magnr_g), len(magnr_R)))
				return None

		else:	
			# convert from PS1 to SDSS (https://arxiv.org/abs/1203.0297)
			gr_ps1 = ps1_mag['g']-ps1_mag['r']
			sdss_mag = {flt : ps1_mag[flt]+self.A0[flt] +self.A1[flt]*(gr_ps1)+self.A2[flt]*(gr_ps1)**2 for flt in ('g','i')}

			gi = (sdss_mag['g']-sdss_mag['i'])
		
			Gaia_from_sdss = sdss_mag['g']-0.098958 -0.6758*gi -0.043274*gi**2 +0.0039908*gi**3 # from Jordi (2010) #https://arxiv.org/abs/1008.0815
	...
	def get_gaia(self, alert):
		...
		ra, dec = np.median(alert.get_values("ranr")), np.median(alert.get_values("decnr"))
		try:
	...
	def gaia_veto(self, alert):		
		...
		if len(gaia_arr) > self.maxGaiamatches:
			self.why = '''number of Gaia matches within 30" is {0}, which is > {1}'''.format(len(gaia_arr), self.maxGaiamatches)
		...
		if gaia_brighest_match<self.brightGaiaMag:
			self.why = '''within {0:0.1f}" of  bright star in Gaia: G={1:0.2f}, which is {2:0.1f}'''.format(self.brightObjDist, gaia_brighest_match, self.brightGaiaMag)

		idxNear = gaia_match_dist < self.singleGaiaDist
		if sum(idxNear)>1:
			self.why = '''number of Gaia matches within {0:0.1f}" is {1}, which is one too many'''.format(self.singleGaiaDist,sum(idxNear))
			self.logger.info(self.why)
			return True
		elif sum(idxNear)==0:
			self.why = '''no Gaia matches within {0:0.1f}"'''.format(self.singleGaiaDist)
			self.logger.info(self.why)
			return False
		...
		if parallax_snr>self.maxParallaxSNR:
			self.why = '''parallax SNR={0:0.1f}, which is > {1:0.1f}'''.format(parallax_snr, self.maxParallaxSNR)
			self.logger.info(self.why)
			return True
		...
		if sg1<self.minSafeSgscore:
			self.logger.info("not vetoing this alert because sgscore={0:0.2f}, which is < {1:0.2f}".format(sg1, self.minSafeSgscore))
			return False
		
		if delta_G < self.minDeltaG:
			self.why = "Gaia - Ground = {0:0.2f}, which is < {1:0.2f}".format(delta_G, self.minDeltaG)
			self.logger.info(self.why)
			return True
	...
	def apply(self, alert):
		...
		if len(sgscore1) == 1:
			...
			sgscore1, sgscore2, sgscore3 = alert_pps["sgscore1"], alert_pps["sgscore2"], alert_pps["sgscore3"]
			distpsnr1, distpsnr2, distpsnr3  = alert_pps["distpsnr1"], alert_pps["distpsnr2"], alert_pps["distpsnr3"]
		...
		if (sgscore2 > self.minSgscore) and (abs(distpsnr2) < self.closePS1dist):
			self.why = "sgscore2 = {0:0.2f}, which is > {1:0.2f}  (distpsnr2={2:0.2f})".format(sgscore2, self.minSgscore, distpsnr2)
			self.logger.info(self.why)
			return None
		...
		if (psmag1['r'] < 0) and \
			not((psmag2['r'] > 0) and (abs(distpsnr2) < self.closePS1dist)):	
				self.why = "1st PS1 match r-band photometry is faulty: mag={0:0.2f} (dist={1:0.2f})".format(psmag1['r'], distpsnr1)
				if distpsnr2 < self.closePS1dist:
					self.why += "; 2nd PS1 match r-mag={0:0.2f} (dist={1:0.2f})".format(psmag1['r'], distpsnr1)
				self.logger.info(self.why)
				return None
		...
		for k in ('g','r','i','z'):
			...
			if (abs(psmag1[k]) < self.brightPS1Mag[k]):
				self.why = "1st PS1 match in {0}-band, mag={1:0.2f}, which is < {2:0.2f} (dist={3:0.2f})".format(k, psmag1[k], self.brightPS1Mag[k], distpsnr1)
				self.logger.info(self.why)
				return None

			if psmag2 is not None: # old schema check
				if (abs(psmag2[k]) < self.brightPS1Mag[k]) and (abs(distpsnr2) < self.brightObjDist):
					self.why = "2nd PS1 match in {0}-band, mag={1:0.2f}, which is < {2:0.2f} (dist={3:0.2f})".format(k, psmag2[k], self.brightPS1Mag[k], distpsnr2)
					self.logger.info(self.why)
					return None

			if psmag3 is not None:
				if (abs(psmag3[k]) < self.brightPS1Mag[k]) and (abs(distpsnr3) < self.brightObjDist):
					self.why = "3rd  PS1 match in {0}-band, mag={1:0.2f}, which is < {2:0.2f} (dist={3:0.2f})".format(k, psmag3[k], self.brightPS1Mag[k], distpsnr3)
					self.logger.info(self.why)
					return None
		...
		if len(jd_arr)<self.minDetections:
			self.why ="only {0} detections pass default filter, but we require at least {1} (in each band)".format(len(jd_arr), self.minDetections)
			self.logger.info(self.why)
			return None
		...
		gc = astropy.coordinates.SkyCoord(ra=ra*astropy.units.deg, dec=dec*astropy.units.deg).galactic
		gal_l, gal_b = gc.galactic.l.deg, gc.galactic.b.deg
		if abs(gal_b)<self.minGalLat:
			self.why = "Too close to the plane; l,b=({0:0.5f} {1:0.5f}), |b|<{2:0.1f}".format(gal_l,gal_b, self.minGalLat)
			self.logger.info(self.why)
			return None
		...
		distnr_arr, magpsf_arr, sigmapsf_arr, rb_arr, fwhm_arr, fid_arr = \
		 map(np.array, zip(*alert.get_ntuples(["distnr","magpsf", "sigmapsf","rb","fwhm", "fid"], filters=self._default_filters)))
		...
		if np.min(magpsf_arr)>self.diffmagLimit:
			self.why = "too faint, min(mag)={0:0.2f}, which is > {1:0.1f}".format(np.min(magpsf_arr), self.diffmagLimit)
			self.logger.info(self.why)
			return None 

		# apply lower limit on flux increase 
		if np.min(magpsf_arr - magnr_arr)>self.maxDeltaMag:
			self.why = 'flux increase too small, min(magpsf-magnr)={0:0.2f}, which is > {1:0.2f}'.format(np.min(magpsf_arr - magnr_arr), self.maxDeltaMag)
			self.logger.info(self.why)
			return None
		...
		ra_arr, dec_arr, ranr_arr, decnr_arr = \
			map(np.array, zip(*alert.get_ntuples(["ra", "dec", "ranr", "decnr"], filters=self._default_filters))) 
	
		offset_sigma = np.clip(0.27 + 1.66 * (sigmapsf_arr-0.1)	, 0.1, 10) 	# equation from the NedStark paper
		...
		for filterid,filtername in zip([1,2],["g","r"]):
			try:
				time,flux=map(np.array,zip(*alert.get_tuples("jd","magpsf",filters=[{"attribute": "candid", "operator": "is not", "value": None},{"attribute": "fid", 'operator': '==', "value": filterid}])))
				fluxerr=np.asarray(alert.get_values("sigmapsf",filters=[{"attribute": "candid", "operator": "is not", "value": None},{"attribute": "fid", 'operator': '==', "value": filterid}]))
				
				sortind=np.argsort(time)
				time=time[sortind]
				flux=flux[sortind]
				fluxerr=fluxerr[sortind]
				results=qso_fit(time, flux, fluxerr, filter=filtername, sys_err=0.0, return_model=True)
		...
		for idx, bnd in zip([idx_g, idx_r],['g','r']):
			...
			if sum(idx)>=self.minDetections:
				...
				mean_dist = 3600 * np.sqrt(np.mean(ra_arr-ranr_arr)**2 + np.mean((dec_arr-decnr_arr)[idx])**2) 
				median_dist = 3600 * np.sqrt(np.median((ra_arr-ranr_arr)[idx])**2 + np.median((dec_arr-decnr_arr)[idx])**2) 
				weighted_dist = 3600 * np.sqrt( self.wmean((ra_arr-ranr_arr)[idx],   snr_weight[idx])**2 + \
										 		self.wmean((dec_arr-decnr_arr)[idx], snr_weight[idx])**2 ) 
					
				if mean_dist < self.maxDeltaRad and mean_dist > self.minDeltaRad:
					self.why = "pass on mean dist={0:0.2f}, band={1}; detections used={2}".format(mean_dist, bnd, sum(idx))
					self.logger.info(self.why)
					return self.on_match_t2_units
				
				if median_dist < self.maxDeltaRad and median_dist > self.minDeltaRad:
					self.why = "pass on median dist={0:0.2f}, band={1}; detections used={2}".format(median_dist, bnd, sum(idx))
					self.logger.info(self.why)
					return self.on_match_t2_units 
				
				if weighted_dist < self.maxDeltaRad and weighted_dist > self.minDeltaRad:
					self.why = "pass on weighted dist={0:0.2f}, band={1}; detections used={2}".format(weighted_dist, bnd, sum(idx))
					self.logger.info(self.why)
					return self.on_match_t2_units

		# if none of the measures of the host-flare pass the cut, reject this alert
		try:
			self.why = "mean/median/weighted dist in {0}-band = ({1:0.2f}/{2:0.2f}/{3:0.2f}), which is > {4:0.2f}".format(bnd, mean_dist, median_dist, weighted_dist, self.maxDeltaRad)
			self.logger.info(self.why)
			return None 
		except NameError:
			self.why = "offset distance not created, likely not enough data passed; number left in r&g band ={0}".format(sum(idx))
			self.logger.info(self.why)
			return None 
