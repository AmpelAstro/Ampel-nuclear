...
dt=[ ('name','U35'),('classification','U35'),
	('photoclass','U35'),('blabber', 'U200'),
	('band','U6'),
	('rms','f8'),('mad','f8'),('chi2','f8'),
	('mjd_peak','f8'),
	('dtime_peak','f8'),('e_dtime_peak','f8'),
	('mag_peak','f8'),('e_mag_peak','f8'),
	('sigma_rise','f8'),('sigma_fade','f8'),
	('e_sigma_rise','f8'),('e_sigma_fade','f8'),
	('n_rise','f8'),('n_fade','f8'),
	('mean_color','f8'),('color_slope','f8'),
	('e_mean_color','f8'),('e_color_slope','f8')]
...
exception_dict = {}
exception_dict['ZTF17aaazdba'] = {'jd_min':2458515} # has early bogus data, so we require jd larger than jd_min
...
def compute(meta_data, lc):
	...
	dtime, flux, flux_err, fid, isdetect, jd0, magnr_dict, mdata = trim_ztf_ampel_data1(lc, logger, exception_dict=edict, oldest_upper_limits = meta_data['oldest_upper_limits'], max_post_peak = meta_data['max_post_peak'])
	lc_data['dtime'], lc_data['flux'], lc_data['flux_err'], lc_data['fid'], lc_data['isdetect'] = dtime, flux, flux_err, fid, isdetect
	...
	fit_params, lc_data, plot_info = flexfit.flex_fit_wclip(dtime,flux, flux_err, fid, logger, isdetect, verbose=False)
	lc_data['jd'] = mdata['jd']
	lc_data['jd0'] = jd0
	lc_data['pid'] = mdata['pid']
	lc_data['programid'] = mdata['programid']
	...
	if 'mag_peak' in fit_params:
		finfo = 't_peak={0:0.1f} ({1}); m_peak={2:0.1f}; rise={3:0.1e}; fade={4:0.1e}; chi2={5:0.1f}'.format(
								fit_params['dtime_peak'],str_em, fit_params['mag_peak'], fit_params['sigma_rise'], fit_params['sigma_fade'], fit_params['chi2'])
	else:
		finfo= 'no fit'
	...
	if 'mean_color' in fit_params:
		...
		finfo += '\ncolor={0:0.2f} +/- {1:0.2f}; slope={2:0.3f} +/- {3:0.3f}'.format(fit_params['mean_color'],fit_params['e_mean_color'], fit_params['color_slope'], fit_params['e_color_slope'])
	...
	return {'fit_params': fit_params, 'lightcurve_data': lc_data, 'plot_info': plot_info}


def trim_ztf_ampel_data1(data_json, logger, oldest_upper_limits=14, 
			filters = [1,2],
			only_ztf = True,
			instruments=['ZTF','SEDM', 'IOO'],
			max_post_peak=200, remove_neg=True, exception_dict=exception_dict):
	'''
	...
	data_dict1 = {key:value for key, value in data_json.items() if len(value)==len(data_json['magnr'])} # everything that isn't a single value

	# I expect to see RuntimeWarnings in this block, median([])=nan
	with warnings.catch_warnings():
		warnings.simplefilter("ignore", category=RuntimeWarning)
		magnr_dict0 = {str(k): np.median(data_json['magnr'][np.where((data_json['fid']==k)&(data_json['magnr'] is not None))[0]]) for k in (1, 2, 3)}
	...
	# some trimming
	if remove_neg:
		itrim0 = (data0['isdiffpos'].values=='t') | (data0['isdiffpos'].values=='1') # only positive detections
		#print('neg:', itrim0)
	else:
	...
	itrim0 &= (data0['jd']-data0['jd'][data0['magpsf'].idxmin()])<max_post_peak # upto xx days post guessed peak
	...
	fid = np.array(['g' if d==1 else 'r' if d==2 else 'i' if d==3 else 'check filter?' for d in data['fid']])
	isdetect = data['magpsf'].values <22
	islim = isdetect==False 

	logger.info('# data point used for fitting {0} (with {1} upper limits)'.format(len(data), sum(islim)))

	flux = mag2flux(data['magpsf'].values)
	flux_lim = mag2flux(data['diffmaglim'].values) # 5 sigma limit for entire image (ie, not including host noise)
	flux[islim] = 0 
	...
	flux_err[islim]= ( (flux_lim[islim]/5)**2 + gain_corr_est**2 * mag2flux(magnr_arr[islim]) )**0.5
